window.env = {
  VITE_SUPABASE_URL: 'https://ajlgjjjvuglwgfnyqqvb.supabase.co',
  VITE_SUPABASE_ANON_KEY: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFqbGdqamp2dWdsd2dmbnlxcXZiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDk1NDMxNjYsImV4cCI6MjA2NTExOTE2Nn0.HPsxr84nkr3Ys7XafPDoU_Z94QFgbT1o1aNfAeaXpRU',
  VITE_ENABLE_REALTIME: 'true',
  VITE_DEBUG_MODE: 'true',
  VITE_APP_URL: window.location.origin,
  VITE_WEBHOOK_URL: `${window.location.origin}/webhook/evolution`,
  VITE_EVOLUTION_API_URL: 'https://webhook.bkcrm.devsible.com.br/api',
  VITE_EVOLUTION_WEBHOOK_URL: 'https://webhook.bkcrm.devsible.com.br',
  VITE_EVOLUTION_API_KEY: '429683C4C977415CAAFCCE10F7D57E11',
  VITE_WEBSOCKET_URL: 'wss://websocket.bkcrm.devsible.com.br',
  NODE_ENV: 'development',
  VITE_API_BASE_URL: window.location.origin,
  VITE_FRONTEND_URL: window.location.origin,
  VITE_LOCAL_WEBHOOK_PORT: '4000',
  VITE_LOCAL_DEV_PORT: '3003',
  VITE_FALLBACK_WEBSOCKET_URL: 'ws://localhost:4000',
  VITE_FALLBACK_API_URL: 'http://localhost:4000'
}; 