...
import { ChatLoadingState } from '../types/chatModal';

interface UseLoadingStateOptions {
  initialState?: Partial<ChatLoadingState>;
...
export const useLoadingState = (options: UseLoadingStateOptions = {}) => {
  ...
  const startLoading = useCallback((stage?: ChatLoadingState['stage']) => {
    setLoadingState({
      isLoading: true,
      error: null,
      progress: 0,
  ...
  const updateProgress = useCallback((progress: number, stage?: ChatLoadingState['stage']) => {
  ...
  const simulateLoading = useCallback(async (
    stages: Array<{ stage: ChatLoadingState['stage']; duration: number; progress: number }>
  ) => {
    startLoading();
