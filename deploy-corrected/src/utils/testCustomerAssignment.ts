  ...
  try {
    ...
    if (verifyTicket.customer_id === testCustomer.id) {
      console.log('✅ [TESTE] Vinculação confirmada no banco!', {
        ...
        customer_name: (verifyTicket.customer as any)?.name,
...
// Adicionar às funções globais para debug
declare global {
  interface Window {
    testCustomerAssignment: typeof testCustomerAssignment;
    testTicketWithCustomerLoading: typeof testTicketWithCustomerLoading;
