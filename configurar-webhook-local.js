// 🔧 Configurador Automático de Webhooks Evolution API - AMBIENTE LOCAL
// Este script configura os webhooks para todas as instâncias ativas

const WEBHOOK_URL = 'http://localhost:4000/webhook/evolution'; // Para desenvolvimento local
...
async function listInstances() {
  ...
  instances.forEach((instance, index) => {
    console.log(`${index + 1}. ${instance.name || instance.instance} - Status: ${instance.state || instance.status || 'Desconhecido'}`);
  });
...
async function main() {
  ...
  try {
    ...
    console.log('\n⚠️ IMPORTANTE: Certifique-se de que o webhook local está rodando em http://localhost:4000');
    ...
    if (successCount > 0) {
      ...
      console.log('2. Verifique os logs do servidor webhook (node webhook-evolution-complete.js)');
      console.log('3. Verifique se o ticket foi criado no CRM');
      console.log('4. URL de teste: http://localhost:4000/webhook/health');
    }
