...
export function useUserDepartment() {
  const [userInfo, setUserInfo] = useState<UserDepartmentInfo | null>(null);
  ...
  const updateUserDepartment = useCallback(async (departmentId: string | null) => {
  ...
  const belongsToDepartment = useCallback((departmentId: string) => {
