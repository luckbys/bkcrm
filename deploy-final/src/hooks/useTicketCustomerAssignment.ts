...
export const useTicketCustomerAssignment = () => {
  ...
  const verifyAssignment = useCallback(async (ticketId: string): Promise<{ customerId: string | null; customerData: any }> => {
    try {
  ...
  const updateCustomerHistory = useCallback(async (customerId: string, ticketId: string) => {
    try {
  ...
  const reloadTicketData = useCallback(async (ticketId: string) => {
    try {
