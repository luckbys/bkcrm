...
export const SectorActions = ({ currentView, onViewChange }) => {
    ...
    const getButtonStyles = (isActive, color) => {
        if (isActive) {
            switch (color) {
                case 'blue':
    ...
    return (_jsx("div", { className: "flex items-center bg-white/40 backdrop-blur-sm rounded-xl p-1 border border-gray-200/40", children: navigationItems.map((item) => {
            const IconComponent = item.icon;
            const isActive = currentView === item.id;
            return (_jsxs(Button, { variant: "ghost", size: "sm", onClick: () => onViewChange(item.id), className: cn("h-9 px-3 rounded-lg border transition-all duration-200 ease-out group", getButtonStyles(isActive, item.color)), children: [_jsx(IconComponent, { className: "w-4 h-4 mr-2 transition-transform group-hover:scale-110 duration-200" }), _jsx("span", { className: "hidden md:inline text-sm font-medium", children: item.label })] }, item.id));
        }) }));
};
