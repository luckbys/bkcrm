...
const Index = () => {
    ...
    const { departments, loading: departmentsLoading, error: departmentsError, refresh: refreshDepartments } = useDepartments();
    ...
    const handleSectorReorder = async (reorderedSectors) => {
    ...
    if (departmentsLoading || (!selectedSector && departments.length > 0)) {
        return (_jsx(FullScreenLoading, { text: "Carregando departamentos...", variant: "primary", showProgress: true, progress: departmentsProgressLoading ? progress : (departmentsLoading ? 75 : 90) }));
    }
    // Se não há departamentos, mostrar mensagem
    if (!departmentsLoading && departments.length === 0) {
        return (_jsx("div", { className: "min-h-screen bg-gray-50 flex items-center justify-center", children: _jsxs("div", { className: "text-center", children: [_jsx("p", { className: "text-gray-600 mb-4", children: "Nenhum departamento encontrado." }), _jsx("button", { onClick: () => refreshDepartments(), className: "bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700", children: "Recarregar" })] }) }));
    }
    return (_jsxs("div", { className: cn("min-h-screen bg-gray-50 flex flex-col", isFullScreen && "fixed inset-0 z-50"), children: [_jsx(Header, { selectedSector: selectedSector, currentView: currentView, onViewChange: handleViewChange, isFullScreen: isFullScreen, onToggleFullScreen: toggleFullScreen, soundEnabled: soundEnabled, onToggleSound: () => setSoundEnabled(!soundEnabled), onOpenAddTicket: () => setShowAddTicketModal(true) }), _jsxs("div", { className: "flex-1 flex pt-36 md:pt-20", children: [_jsx(Sidebar, { onDepartmentSelect: handleSectorChange, selectedDepartmentId: selectedSector?.id, className: "", onCollapsedChange: setSidebarCollapsed }), _jsx(MainContent, { selectedSector: selectedSector, currentView: currentView, onViewChange: handleViewChange, isLoading: isLoading, sidebarCollapsed: sidebarCollapsed, onOpenAddTicket: () => setShowAddTicketModal(true) })] }), showImageModal && (_jsx(ImagePasteModal, { onClose: () => {
                    setShowImageModal(false);
                    setPastedImage(null);
                }, image: pastedImage, onSend: (comment, isInternal) => {
                    ...
                    setShowImageModal(false);
                    setPastedImage(null);
                } })), showAddTicketModal && (_jsx(AddTicketModal, { sector: selectedSector, onClose: () => setShowAddTicketModal(false), onSave: async (ticketData) => {
                    try {
                        const newTicketData = {
