    ...
    try {
      ...
      const { data, error: supabaseError } = await supabase
        ...
        .select(`
          ...
          created_at,
          updated_at,
          responsible_agent:profiles!responsible_agent_id(name)
        `)
        .eq('is_active', true)
        .order('created_at', { ascending: false })
        .returns<CustomerFromDB[]>();
      ...
      const formattedCustomers: Customer[] = (data || []).map(customer => ({
        ...
        lastInteraction: customer.last_interaction || customer.created_at,
        totalOrders: customer.total_orders || 0,
        totalValue: customer.total_value || 0,
        averageTicket: customer.average_ticket || 0,
        responsibleAgent: customer.responsible_agent?.name || '',
      ...
      if (existingCustomer) {
        ...
        const { error: insertError } = await supabase
          ...
          .insert({
            ...
            responsible_agent_id: user.id, // Agora garantimos que o perfil existe
