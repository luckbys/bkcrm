...
export const usePhoneValidationAndSearch = () => {
    ...
    const searchCustomerPhone = useCallback(async (ticketId, customerName) => {
        setIsSearching(true);
        try {
    ...
    const updateTicketWithPhone = useCallback(async (ticketId, phoneData) => {
        try {
            if (!phoneData.found || !phoneData.phone)
                return false;
    ...
    // Função principal
    const validateAndSearchPhone = useCallback(async (ticketId, currentPhone, customerName) => {
