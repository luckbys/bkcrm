...
export class WebhookSimulator {
     ...
     */
    static startConversationSimulation(phone, instanceName = 'financeiro-encontra') {
        const messages = [
     ...
     */
    static showNotification(title, content) {
        // Tentar usar o sistema de toast se disponível
        if (typeof window !== 'undefined' && window.showToast) {
            window.showToast(title, content);
        }
        else {
    ...
    static async clearSimulationData() {
        try {
            ...
            const simulationTickets = autoTickets.filter(ticket => ticket.metadata?.evolution_message_id?.startsWith('sim_'));
...
window.WebhookSimulator = WebhookSimulator;
