...
export class TicketRoutingService {
   ...
   */
  static async routeMessage(message: IncomingMessage): Promise<TicketRoutingResult> {
    try {
   ...
   */
  private static async findOpenTicketByPhone(phone: string): Promise<any | null> {
    try {
   ...
   */
  private static async createTicketAutomatically(message: IncomingMessage): Promise<string | null> {
    try {
   ...
   */
  private static async addMessageToTicket(ticketId: string, message: IncomingMessage): Promise<void> {
    try {
      const messageData = {
        ticket_id: ticketId,
   ...
   */
  static async simulateIncomingMessage(testMessage: Partial<IncomingMessage>): Promise<TicketRoutingResult> {
    const defaultMessage: IncomingMessage = {
   ...
   */
  static async getAutoCreatedTickets(limit: number = 10): Promise<any[]> {
    try {
      const { data: tickets, error } = await supabase
        .from('tickets')
