...
interface MessageUpsertDiagnostic {
  step: string;
...
export const fixMessagesUpsert = async (): Promise<void> => {
  console.log('🔧 [UPSERT-FIX] Diagnosticando endpoint /messages-upsert...');
  console.log('='.repeat(60));
  
  const diagnostics: MessageUpsertDiagnostic[] = [];
  ...
  (window as any).lastUpsertDiagnostics = diagnostics;
...
export const testMessageUpsertFlow = async (): Promise<void> => {
...
export const showUpsertLogs = (): void => {
  const diagnostics = (window as any).lastUpsertDiagnostics;
  ...
  diagnostics.forEach((diag: MessageUpsertDiagnostic, index: number) => {
...
(window as any).fixMessagesUpsert = fixMessagesUpsert;
(window as any).testMessageUpsertFlow = testMessageUpsertFlow;
(window as any).showUpsertLogs = showUpsertLogs;
