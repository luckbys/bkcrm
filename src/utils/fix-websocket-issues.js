...
export const testCompleteFlow = async (ticketId = 'test-ticket-' + Date.now()) => {
    console.log('🧪 [TEST] Testando fluxo completo de mensagens...');
    try {
        // 1. Join no ticket
        console.log('1️⃣ Fazendo join no ticket...');
        const joinResult = await window.fixedJoinTicket?.(ticketId);
        if (joinResult) {
            console.log('✅ Join realizado com sucesso');
            // 2. Enviar mensagem
            console.log('2️⃣ Enviando mensagem de teste...');
            const sendResult = await window.fixedSendMessage?.(ticketId, 'Mensagem de teste do fluxo completo', false);
            if (sendResult) {
