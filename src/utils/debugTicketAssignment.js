...
export const debugTicketAssignment = async (ticketId) => {
    ...
    try {
      ...
      `)
        ...
        if (fullTicket.customer) {
            const customerData = fullTicket.customer;
...
export const debugCurrentTicket = () => {
    ...
    try {
        ...
        const keys = Object.keys(localStorage).filter(key => key.includes('ticket') || key.includes('chat') || key.includes('customer'));
...
export const forceTicketReload = async (ticketId) => {
