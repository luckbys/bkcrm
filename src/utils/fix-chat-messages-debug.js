 ...
 */
const forceReloadMessages = async (ticketId) => {
 ...
 */
const detectTicketDuplication = async () => {
 ...
 */
const fixTicketDuplication = async (phone, keepLatest = true) => {
    ...
    try {
        ...
        const closePromises = duplicateTickets.map(ticket => supabase
            .from('tickets')
            .update({
            status: 'closed',
 ...
 */
const createTestMessage = async (ticketId) => {
 ...
 */
const fixAllProblems = async (ticketId) => {
...
function analyzePerformance() {
    ...
    if (window.wsSocket) {
        ...
        performance.websockets.status = window.wsSocket.connected ? 'connected' : 'disconnected';
    }
...
// Adicionar ao window
window.fixChatSystem = fixChatSystem;
