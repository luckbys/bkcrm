...
export const diagnoseDatabaseIssues = async () => {
    ...
    try {
        ...
        const { data: duplicates } = await supabase
            ...
            .eq('is_default', true);
        if (duplicates) {
            const deptCounts = duplicates.reduce((acc, curr) => {
                acc[curr.department_id] = (acc[curr.department_id] || 0) + 1;
                return acc;
            }, {});
            const duplicateCount = Object.values(deptCounts).filter((count) => count > 1).length;
            if (duplicateCount > 0) {
...
if (typeof window !== 'undefined') {
    ...
    window.diagnoseDatabase = diagnoseDatabaseIssues;
