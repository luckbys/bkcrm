...
interface AutoLinkResult {
  ...
  customer?: Customer;
  confidence: number;
  method: 'exact_match' | 'phone_match' | 'email_match' | 'fuzzy_match' | 'created_new';
  reason: string;
}
...
export const useCustomerAutoLink = () => {
  ...
  const extractClientDataFromTicket = useCallback((ticket: any): ClientData => {
    if (!ticket) return {};
  ...
  const findExistingCustomer = useCallback(async (clientData: ClientData): Promise<{ customer: Customer | null; confidence: number; method: string }> => {
    try {
      ...
      const formattedCustomers: Customer[] = customers.map(profile => {
      ...
      if (clientData.phone) {
        ...
        const phoneMatches = formattedCustomers.filter(customer => {
          ...
          return customerPhone && phoneClean && (
      ...
      if (clientData.email && !clientData.email.includes('@auto-generated.com')) {
        const emailMatch = formattedCustomers.find(customer => 
          customer.email.toLowerCase() === clientData.email?.toLowerCase()
        );
      ...
      if (clientData.name) {
        const nameMatch = formattedCustomers.find(customer => 
          customer.name.toLowerCase() === clientData.name?.toLowerCase()
        );
  ...
  const createNewCustomer = useCallback(async (clientData: ClientData): Promise<Customer | null> => {
    try {
  ...
  const autoLinkCustomerToTicket = useCallback(async (ticket: any): Promise<AutoLinkResult> => {
    try {
  ...
  const enrichCustomerWithTicketData = useCallback(async (customerId: string, ticketData: any): Promise<boolean> => {
    try {
...
function calculateNameSimilarity(name1: string, name2: string): number {
  ...
  for (let i = 0; i <= cleanName1.length; i++) matrix[0][i] = i;
  for (let j = 0; j <= cleanName2.length; j++) matrix[j][0] = j;
